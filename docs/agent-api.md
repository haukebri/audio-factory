...
### Noise reduction and fades

Cut requests accept optional `noise_reduction_db` from 0 to 24 (default 0, Off). Processing always starts from the preserved original. Ordinary cuts use smooth 10 ms fade-in and 100 ms fade-out, scaled for short clips; loop exports keep their seamless join. Saved evidence records the cleanup parameters and effective fade durations. Existing saved versions remain unchanged.

`POST /studio/candidates/:candidateId/denoise-preview` accepts `{ "noise_reduction_db": 12 }` using the same authentication/CSRF rules as other Studio POST requests. It returns `application/octet-stream`: interleaved little-endian float32 PCM of the full preserved source, with `X-Audio-Sample-Rate` and `X-Audio-Channels` headers. Noise reduction measures a quiet-window noise profile, compensates its filter delay and preserves frame count. The editor prepares 24 dB cleanup once and uses the shared `mixNoiseReduction` renderer to adjust strength locally; saved cuts use the identical mix. This preview creates no candidate or selection; apply the normal trim/loop and level renderer to audition the export. At most two previews run concurrently; excess requests receive 429, and disconnecting cancels processing.

Cuts accept optional `pitch_semitones` (−12…12, default 0), `speed` (0.5…2, default 1), `reverse` (boolean, default false), `low_cut_hz` (0/off or 20…1000), and `high_cut_hz` (0/off or 1000…20000). Enabled low cut must be below high cut. Source bounds remain original timestamps. The `edit` evidence object records settings, `ffmpeg-edit-v1` processing version and pre-loop output frames. Loop evidence uses transformed frame coordinates when `edit` is present; `bounds` always references original source frames.

Studio's CSRF-protected `POST /studio/candidates/:id/edit-preview` accepts source bounds and those five effect parameters. It returns concatenated dry then maximum-cleanup f32le stems, with `X-Audio-Sample-Rate`, `X-Audio-Channels`, and `X-Audio-Frames` (per stem). Both stems are transformed identically before the shared noise-strength mix and final fades/normalization/level. Previews do not create candidates. Exports use the same pipeline. FFmpeg resampling and tempo adjustment provide independent pitch/speed without an additional runtime dependency; large shifts can introduce stretching artifacts.
