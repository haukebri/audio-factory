...
4. **Noise reduction** starts Off. Increase its 0–24 dB slider to reduce steady background noise; lower it if wanted detail disappears. The editor prepares cleanup from a quiet part of the recording once; slider changes then update the waveform and playing preview locally, without saving or another server request. Noise reduction and Level sit side by side. **Play** includes the same cleanup, normalization and Level as **Save trim**. Ordinary clips always receive a smooth 10 ms fade-in and 100 ms fade-out, shortened proportionally for tiny clips. The dashed edge envelopes show these fades. Saving creates and selects a new version; **Download** uses that saved version. The original remains unchanged.
5. For continuous audio, enable **Loop** in the editor and adjust **Crossfade (seconds)**. **Loop details** offers equal-power or equal-gain curves. **Preview loop / Stop loop** auditions the proposed processed waveform repeatedly without saving; listen for at least three cycles. The original waveform cursor stays stationary during this processed audition. **Save loop** creates and selects that version. Loop exports retain a seamless join without outer fades; playback fades only when starting or stopping. Usable native loops keep their timing until repair controls change.
...
A completed import becomes another numbered audio take labelled **YouTube**. It initially preserves the full chosen interval as a non-loop clip. **Trim** opens Studio's normal editor: local second zero is the beginning of the imported interval. Noise reduction, Level, fades, loop/crossfade processing and saved versions work as usual. Imports do not choose winners; saving an edit selects its new version. Use **Use this take** to select the exact version for Library and **Export winners**. Prompt, generation-duration and loop drafts remain unchanged.
...
The **Edit** panel keeps selection handles on original source timestamps. Pitch (−12 to +12 semitones) and Speed (0.5×–2×) work independently. Reverse plays the selection backwards. Under Clean sound, Low cut removes low frequencies and High cut removes high frequencies; both default to Off. Reset effects restores neutral pitch, speed, filters, reverse, cleanup and level without changing the selection or loop settings.

Pitch, speed, reverse and tone changes briefly show “Updating preview…” while processing. The existing preview keeps playing, then switches smoothly. The single source waveform provides selection and seeking; no second waveform appears when effects change. Noise reduction and level still update locally once preparation finishes. Failed edits retain the last preview, identify it as outdated, and offer Retry; save is enabled only after current settings render successfully. Automatic fades and loop crossfades operate in output time. Saved edits retain the original source and restore their settings when reopened.
