...
5. For continuous audio, enable **Loop** in the editor and adjust **Crossfade (seconds)**. **Loop details** offers equal-power or equal-gain curves. **Preview loop** repeats the proposed processed waveform without saving; listen for at least three cycles. **Save loop** creates that version without changing the winner. Usable native loops keep their timing until repair controls change.
