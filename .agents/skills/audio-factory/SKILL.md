...
Set `loop: true` on each batch sound when the requested asset should repeat continuously, such as rain, forest or shoreline ambience. For workflow JSON and `/studio/jobs`, put it in `request.loop`; `make` accepts `loop` directly in its request JSON. Omitted/false new requests keep ordinary one-shot behavior. Regeneration omission inherits the sound's latest setting; explicit false disables it. Recreation inherits the selected version's setting unless explicitly overridden. Preserve the effective flag with the saved request/key; a changed loop intent requires a new key.

Duration is the source-generation budget, not an exact final loop length. Trimming and overlap may shorten it; use the winners manifest's actual `duration_seconds` and `loop` when integrating, and configure the consuming player to repeat the approved WAV. Do not pad, stretch, regenerate or reprocess the selected asset to claim the requested duration.

At the human handoff, direct loop reviewers to **Trim → Loop → Preview loop** and ask them to hear at least three cycles, including texture changes, bird-call repetition and wave timing. **Crossfade (seconds)** adjusts the blend; **Loop details** offers curve selection. **Save loop** creates a version; the human must still choose **Use this take**. Preview alone neither saves nor selects. Signal checks do not replace this listening review. Detailed defaults and cut options are in [usage](../../../docs/usage.md#continuous-loops).
...
Non-loop requests select the first active region, apply fades and peak-normalize to −3 dBFS. Loop requests use the continuous original: local audio gets conservative edge trimming and rotation/crossfading; usable native ElevenLabs loops retain their timing with global gain only. Loop outputs receive no outer fades. Complete originals and failed takes are retained. Inspect signal evidence and listen when available; never describe signal success as semantic or human acceptance. There is no listening-model QA or CLAP execution. Semantic execution options return migration errors.

Studio supports fast comparison and **Use this take**, saving the exact preferred candidate and append-only changes without rejecting alternatives. **Recreate with ElevenLabs** makes one paid request from that take's exact generated prompt, original duration and effective loop intent. It needs `ELEVEN_KEY` in the checkout's ignored `.env`. Never expose the key or private Studio token. Paid generations require task authorization; uncertain submissions are never automatically replayed.
