...
const open = () => {
  evaluate(`(() => { closeEditor(); const row = document.querySelector('#batch-results [data-clip]'); trim(candidates.find(c => c.candidate_sha256 === row.dataset.clip), row); window.editor = document.querySelector('.trim-editor'); window.player = editor.querySelector('audio'); window.play = editor.querySelector('.actions button'); })()`);
};
...
try {
  ...
  check(`Math.abs(parseFloat(editor.querySelector('.trim-playhead').style.left)/100*3-player.currentTime)<.08`);
  ...
  evaluate('play.click()'); wait('!player.paused'); evaluate('seek(.7)'); check('!player.paused && player.currentTime>=2.09');
  ...
  check(`(async()=>{const p=player,e=editor;await refresh();return p===document.querySelector('.trim-editor audio')&&e===document.querySelector('.trim-editor')&&!p.paused;})()`);
  ...
  wait('Boolean(loopNode)'); check('player.paused && other.paused');
  const loopCursor=evaluate("editor.querySelector('.trim-playhead').style.left");
  check('loopNode.loop'); evaluate('loopButton.click()'); check('!loopNode && !loopContext');
  check("editor.querySelector('.trim-playhead').style.left",loopCursor);
  evaluate('loopButton.click()'); wait('Boolean(loopNode)'); evaluate('play.click()'); wait('!player.paused'); check('!loopNode && !loopContext');
  evaluate('loopButton.click()'); wait('Boolean(loopNode)'); key('end','ArrowLeft'); check('!loopNode && player.paused');
  ...
  check('player.paused && !loopNode && !loopContext && frames.size===0 && !document.querySelector(".trim-editor")');
